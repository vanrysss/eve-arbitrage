...
with open('sql/item/insert_item.sql', 'r') as item_insert_file:
    item_insert_query = item_insert_file.read()
...
market_response, market_content = h.request("https://esi.tech.ccp.is/dev/markets/groups/?datasource=tranquility","GET")
m_pretty = literal_eval(market_content.decode('utf-8'))
for group in m_pretty:
    group_response, group_content = h.request("https://esi.tech.ccp.is/dev/markets/groups/"+str(group) +"/?datasource=tranquility&language=en-us", "GET")
    group_pretty = literal_eval(group_content.decode('utf-8'))
    print(group_pretty)
    if group_pretty['types']:
        #persist item data
        for item in group_pretty['types']:
            print(item)
            item_response, item_content = h.request("https://esi.tech.ccp.is/dev/universe/types/" + str(item)+"/?datasource=tranquility&language=en-us","GET")
            item_pretty = json.loads(item_content.decode('utf-8'))
            if item_pretty['published']:
                cursor.execute(item_insert_query, (item_pretty['name'], item_pretty['type_id'], item_pretty['description']))
                conn.commit()
